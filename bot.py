...
@app.on_message(filters.photo & filters.create(lambda _, message: message.photo[-1].mime_type == "image/jpeg"))
async def handle_jpeg_photo(client, message):
...
async def start_command(client, message):
    ...
    start_keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("Help", callback_data="help"),  # Add Help option
             InlineKeyboardButton("About", callback_data="about")],  # Add About option
        ...
        if format_buttons:
            ...
            if upload_mode == "video":
                await message.reply_video(
                    video=download_url,
                    caption=format_message[:1024] + "..." if len(format_message) > 1024 else format_message,  # Limit caption to 1024 characters
                    thumb=thumbnail_url,
...
@app.on_message(filters.command("show_thumbnail"))
...
@app.on_message(filters.command("delete_thumbnail"))
...
async def settings_command(client, message):
    ...
    delete_thumbnail_callback = f"delete_thumbnail_{user_id}"  # Define the back button callback data
...
# Define the "Help" callback
@app.on_callback_query(filters.regex(r"^help_\d+$"))
async def help_callback(client, callback_query):
    user_id = int(callback_query.data.split("_")[-1])

    # Define the help message
    help_message = "This is the URL Uploader Bot. It can help you upload videos and files from URLs. Just send a valid URL, and the bot will guide you through the process."

    # Send the help message to the user
    await app.send_message(user_id, help_message)

# Define the "About" callback
@app.on_callback_query(filters.regex(r"^about_\d+$"))
async def about_callback(client, callback_query):
    user_id = int(callback_query.data.split("_")[-1])

    # Define the about message
    about_message = "URL Uploader Bot v1.0\nCreated by YourName\nPowered by Python, Pyrogram, and yt_dlp."

    # Send the about message to the user
    await app.send_message(user_id, about_message)
